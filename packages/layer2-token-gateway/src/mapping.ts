...
export function handleDeposit(event: DepositFinalizedEvent): void {
  ...
  const depositId = event.transaction.hash.toHex();
  ...
  const testGatewayId = addressToId(event.address);
  const testTokenId = addressToId(event.params.l1Token);
  deposit.exitInfo = getJoinId(testGatewayId, testTokenId);
